...
export default class PlanetSignif extends Component{
...
//    this.showLoading()
        ...
        fetch(url, {
            ...
            .then((responseJson) => {
//          console.log('adsd'+JSON.stringify(responseJson))
  //             this.hideLoading()
                if (responseJson.status == true) {

                    var obj = {}
                    var arr = responseJson.responseData.planetSignificator
                    // for(let i = 0; i< arr.length; i++){
                    //     obj=arr[i]
                    // }

                this.setState({response: responseJson.responseData.planetSignificator,
                },()=>{
  ...
  renderThis=({item,index})=>{
    return(
    <Text style={{fontFamily:'Nunito-Bold',width:'100%',backgroundColor:'transparent', fontSize:14, color:'black', marginLeft:wp(1.5), textAlign:'left'}}>{item},</Text>

        )
  }

  _renderSignifOne=({item,index})=>{
//    console.log(Array.isArray((item.level4)))
    var dec_render;
    if(Array.isArray((item.level4))){
        dec_render = 
                <FlatList data={item.level4}
    ...
    dec_render =  
    <Text style={{fontFamily:'Nunito-Bold',width:'100%',backgroundColor:'transparent', fontSize:14, color:'black', marginLeft:wp(1.5), textAlign:'left'}}>{item.level4}</Text>


    }

    return(
        <View style={{width:'100%', backgroundColor:'white', height:hp(6.5), alignItems:'center', flexDirection:'row'}}>
        {dec_render}
        </View>
        )
  ...
  return(
    ...
    Level 3 - House owned by the star lord of the planet{`\n`}
    Level 4 - House owned by the planet itself{`\n`}
...
{/*    <FlatList style={{width:'15%', backgroundColor:'pink'}}
    data={this.state.response}
    horizontal={false}
    renderItem={this._renderSignifOne}
    keyExtractor = { (item, index) => index.toString() }
